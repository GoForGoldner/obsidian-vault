...
START
Basic
display: none vs visibility: hidden: Compare `display: none`, `visibility: hidden`, and `opacity: 0` on two axes: does the element keep its layout space, and can it still be clicked?
...
START
Basic
display: none vs visibility: hidden: You want to hide an element but keep the exact gap it occupied so the layout doesn't shift. Which property?
...
START
Basic
display: none vs visibility: hidden: Which "hide" technique still lets the hidden element receive clicks, and how do you stop that?
...
START
Basic
display: none vs visibility: hidden: Why can't you smoothly animate hiding with `display: none`, and what do you use instead for a fade-out?
