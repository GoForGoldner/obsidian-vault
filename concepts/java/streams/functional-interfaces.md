...
START
Basic
Functional Interfaces: What exactly makes an interface a *functional interface*?
...
START
Basic
Functional Interfaces: Gotcha: does declaring `equals`/`toString` abstractly break a `@FunctionalInterface`?
...
START
Basic
Functional Interfaces: Match the abstract method name: `Supplier`, `Consumer`, `Function`, `Predicate`.
...
START
Basic
Functional Interfaces: What's the difference between `Function.andThen(g)` and `Function.compose(g)`?
...
START
Basic
Functional Interfaces: You need `int -> boolean` without boxing. Which functional interface?
...
START
Basic
Functional Interfaces: How do `UnaryOperator<T>` and `BinaryOperator<T>` relate to `Function`/`BiFunction`?
...
START
Basic
Functional Interfaces: Which combinator methods do `Predicate` and `Consumer` provide?
...
START
Basic
Functional Interfaces: What's the abstract method of `ToIntFunction<T>` vs `IntFunction<R>`?
