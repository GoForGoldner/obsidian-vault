...
START
Basic
Lambdas and Method References: What determines the type of a lambda expression?
...
START
Basic
Lambdas and Method References: Why can a lambda only capture *effectively final* local variables?
...
START
Basic
Lambdas and Method References: Gotcha: what does `this` refer to inside a lambda?
...
START
Basic
Lambdas and Method References: Distinguish unbound (`String::toUpperCase`) from bound (`str::toUpperCase`) instance method refs.
...
START
Basic
Lambdas and Method References: You see `ArrayList::new`. What kind of method reference is it and what does it mean?
...
START
Basic
Lambdas and Method References: When can you NOT use a method reference and must use a lambda instead?
...
START
Basic
Lambdas and Method References: Why does `(a, b) -> a.compareTo(b)` shorten to `String::compareTo` but not `a::compareTo`?
...
START
Basic
Lambdas and Method References: Lambda gotcha: which block-body lambdas need an explicit `return`?
...
START
Basic
Lambdas and Method References: Why pass `this::search` (not `search(key)`) to `map.computeIfAbsent(key, fn)`?
