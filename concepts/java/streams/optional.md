...
START
Basic
Optional: Distinction: `orElse(x)` vs `orElseGet(supplier)` — the exam's favorite Optional trap.
...
START
Basic
Optional: What's the difference between `Optional.of(x)` and `Optional.ofNullable(x)`?
...
START
Basic
Optional: What does `Optional.get()` do when the Optional is empty?
...
START
Basic
Optional: When do you use `flatMap` instead of `map` on an Optional?
...
START
Basic
Optional: What does `optional.filter(predicate)` return?
...
START
Basic
Optional: `ifPresent` vs `ifPresentOrElse` — what's the difference?
...
START
Basic
Optional: Gotcha: where should you NOT use `Optional`?
...
START
Basic
Optional: Which Optional types avoid boxing, and where do they come from?
