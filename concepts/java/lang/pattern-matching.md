...
START
Basic
Pattern Matching: What does `if (o instanceof String s)` give you over plain `instanceof`?
...
START
Basic
Pattern Matching: Why does `s` compile after this line?<br>`if (!(o instanceof String s)) return; s.length();`
...
START
Basic
Pattern Matching: How do you deconstruct a record in a pattern, and can patterns nest?
...
START
Basic
Pattern Matching: In a pattern switch, how do you add a condition to a case, and what's the ordering rule?
...
START
Basic
Pattern Matching: What is "dominance" in a pattern switch and what breaks if you get it wrong?
...
START
Basic
Pattern Matching: How does a pattern `switch` handle `null`, and how does that differ from a classic switch?
...
START
Basic
Pattern Matching: When can a pattern switch over a `sealed` type omit `default`?
...
START
Basic
Pattern Matching: As of Java 26, are primitive type patterns (e.g. `o instanceof int`, `switch` on `double`) standard or preview?
