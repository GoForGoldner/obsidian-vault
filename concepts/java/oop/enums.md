...
START
Basic
Enums: What does `Enum.valueOf("NOPE")` throw when no constant matches? And on `null`?
...
START
Basic
Enums: Why is an enum the preferred way to write a singleton?
...
START
Basic
Enums: Can an enum extend a class? Implement an interface?
...
START
Basic
Enums: You want each enum constant to behave differently for one method. How?
...
START
Basic
Enums: What visibility does an enum constructor have, and how often does it run?
...
START
Basic
Enums: In a `switch (planet)`, how do you write the case labels?
...
START
Basic
Enums: You're mapping/setting keyed by enum values — which collections, and why?
...
START
Basic
Enums: Why avoid persisting `ordinal()` to a database?
