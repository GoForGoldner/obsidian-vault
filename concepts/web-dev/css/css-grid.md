...
START
Basic
CSS Grid: You need rows and columns that line up with each other — a true grid like a dashboard or photo wall. Which layout system, and why not flexbox?
...
START
Basic
CSS Grid: Write the CSS for a container with three equal-width columns separated by 16px gaps.
...
START
Basic
CSS Grid: Write the CSS for a responsive gallery: as many columns as fit at a minimum of 200px each, each stretching to fill, wrapping otherwise.
...
START
Basic
CSS Grid: In `repeat(auto-fit, ...)` vs `repeat(auto-fill, ...)`, what's the behavioral difference when there aren't enough items to fill the row?
...
START
Basic
CSS Grid: Write the CSS to make a single grid item span two columns.
