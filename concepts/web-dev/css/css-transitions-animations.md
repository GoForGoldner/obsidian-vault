...
START
Basic
CSS Transitions and Animations: What's the fundamental difference between a `transition` and an `animation`?
...
START
Basic
CSS Transitions and Animations: Write the CSS to make a button smoothly scale up to 1.05× over 0.2s when hovered.
...
START
Basic
CSS Transitions and Animations: Write a `@keyframes` rule named `spin` that rotates a full turn, plus the line that runs it forever at constant speed over 1s.
...
START
Basic
CSS Transitions and Animations: For smooth, jank-free animation, which two properties should you prefer to animate, and why?
...
START
Basic
CSS Transitions and Animations: Your hover transition snaps instantly with no animation even though `:hover` changes the property. What's the usual mistake?
