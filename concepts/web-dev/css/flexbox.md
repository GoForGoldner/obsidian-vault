...
START
Basic
Flexbox: You need to lay items out along a single line (a row or a column) and distribute the space between them. Which layout system, and why not the other one?
...
START
Basic
Flexbox: Write the CSS to perfectly center a single child both horizontally and vertically using flexbox.
...
START
Basic
Flexbox: In a default flex row, which property controls horizontal placement and which controls vertical — and what happens to them with `flex-direction: column`?
...
START
Basic
Flexbox: Write the CSS to push one item to the far left and another to the far right of the same bar.
...
START
Basic
Flexbox: What does the shorthand `flex: 1` expand to, and what's the effect?
...
START
Basic
Flexbox: By default a flex container forces everything onto one line (items shrink/overflow). What's the one declaration that lets items wrap to new lines?
