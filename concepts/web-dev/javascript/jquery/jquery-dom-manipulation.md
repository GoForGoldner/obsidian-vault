...
START
Basic
jQuery DOM Manipulation: What's the difference between calling `$el.text()` and `$el.text('hi')`?
...
START
Basic
jQuery DOM Manipulation: You need a checkbox's current checked state in jQuery. Do you use `.attr('checked')` or `.prop('checked')`, and why?
...
START
Basic
jQuery DOM Manipulation: When does `.html()` bite you compared to `.text()`?
...
START
Basic
jQuery DOM Manipulation: Write the jQuery to read the value of `<input id="name">` and then clear it.
...
START
Basic
jQuery DOM Manipulation: Map these jQuery class methods to their vanilla `classList` equivalents: addClass, removeClass, toggleClass, hasClass.
...
START
Basic
jQuery DOM Manipulation: Why does `.remove()` differ from just emptying an element's HTML?
