...
START
Basic
Promises: JS is single-threaded, so how does it handle long-running work without freezing?
...
START
Basic
Promises: What three states can a Promise be in, and how many times can it settle?
...
START
Basic
Promises: What's the difference between `Promise.all` and `Promise.allSettled` on failure?
...
START
Basic
Promises: When does `.finally()` run on a promise, and does it receive the resolved value?
...
START
Basic
Promises: A `setTimeout(fn, 0)` and a `Promise.resolve().then(fn2)` are both pending. Which callback runs first?
...
START
Basic
Promises: Why does returning a promise inside `.then(cb)` not nest into a promise-of-a-promise?
