...
START
Basic
Rules of Hooks: State the two Rules of Hooks.
...
START
Basic
Rules of Hooks: Mechanically, why does calling a hook inside an `if` break React?
...
START
Basic
Rules of Hooks: You need an effect to run only when `loggedIn` is true. Where does the `if` go — around the `useEffect` call or inside it?
...
START
Basic
Rules of Hooks: Can you call `useState` inside an `onClick` handler? Why or why not?
...
START
Basic
Rules of Hooks: What tool catches Rules-of-Hooks violations, and how does it know a function is a hook?
...
START
Basic
Rules of Hooks: Is it OK to have an early `return null;` in a component, given the top-level rule?
