...
START
Basic
useRef: What does `useRef(initial)` return, and what's the one property you read/write?
...
START
Basic
useRef: The core distinction: what happens to rendering when you change `ref.current` vs when you call a `useState` setter?
...
START
Basic
useRef: How do you grab the actual DOM element for an `<input>` so you can call `.focus()`?
...
START
Basic
useRef: Why initialize a DOM ref with `null` and type it `useRef<HTMLInputElement>(null)`?
...
START
Basic
useRef: When should you use a ref instead of state to hold a value (e.g. a timer id or previous value)?
