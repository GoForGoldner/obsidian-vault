...
START
Basic
useEffect: Contrast the three dependency-array forms of useEffect: omitted, `[]`, and `[a]`. When does each effect run?
...
START
Basic
useEffect: What is a useEffect cleanup function, when does React call it, and what's it for?
...
START
Basic
useEffect: You wrote an effect with `[]` that references `count`, and it always sees `count` as its initial value. What's this bug called and why does it happen?
...
START
Basic
useEffect: `useEffect(() => setCount(count + 1), [count])` freezes the page. Why?
...
START
Basic
useEffect: An effect with deps `[options]` re-runs every render even though "nothing changed." `options` is `{ sort: "asc" }` defined inline. Why?
...
START
Basic
useEffect: Why does useEffect run *after* render rather than during it?
