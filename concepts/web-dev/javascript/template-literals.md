...
START
Basic
Template Literals: What character delimits a template literal, and what two things does it give you over a normal `"string"`?
...
START
Basic
Template Literals: Can you put arbitrary expressions inside `${}`, or only variable names?
...
START
Basic
Template Literals: Gotcha: what does `` `Value: ${obj}` `` produce when `obj` is a plain object?
...
START
Basic
Template Literals: You see `` css`color: ${c}` `` — a function name immediately before a backtick string. What is this?
...
START
Basic
Template Literals: Write a template literal greeting `name` and pluralizing based on `count`.
